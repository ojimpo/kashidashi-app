...
kashidashi 自体はシンプルな REST API と Web UI だけを持ちます。スクレイピング、リッピング連携、リマインダー通知といったロジックは **外部のエージェントやスクリプト** に任せ、アプリは「データを受け取って保存する」「データを返す」「見せる」ことだけに集中します。
...
**OpenClaw** は自分が開発している AI エージェントです（別リポジトリ）。kashidashi の自動化を担います。

### 新規貸出の自動登録

1. OpenClaw が図書館の Web サイトにログインして貸出一覧をスクレイピング
